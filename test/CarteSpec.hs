...
tailleSpec = do 
    describe "Verifie si le nombre de cases correspond à la taille de la carte" $ do
        it "Carte avec un contenu vide mais une taille " $ do 
            C.prop_TailleCarte (C.Carte 100 100 (C.C 50 50) M.empty)
            `shouldBe` False
        it "Une bonne taille de carte (creer manuellement)"  $ do
            C.prop_TailleCarte (C.Carte 50 50 (C.C 0 0) (M.singleton (C.C 0 0) C.Vide))
            `shouldBe` True
...
caseSpec = do
    describe "Verifie les coordonnées de chaque cases et si la carte est bien entouré de mur" $ do
        it "Carte avec une case (un mur)" $ do 
            C.prop_CaseCarte (C.Carte 50 50 (C.C 0 0) (M.singleton (C.C 0 0) C.Horizontal))
            `shouldBe` True
        it "Carte avec une case (Vide)" $ do 
            C.prop_CaseCarte (C.Carte 50 50 (C.C 0 0) (M.singleton (C.C 0 0) C.Vide))
            `shouldBe` False
        it "Carte avec une case (coordonnée negatif)" $ do 
            C.prop_CaseCarte (C.Carte 50 50 (C.C 0 0) (M.singleton (C.C (-10) (-10)) C.Vide))
            `shouldBe` False
        it "Une bonne carte en lecture" $ do
...
porteSpec = do
    describe "Verifie que les portes sont supportées par des murs" $ do
        it "Carte avec une case (porte)" $ do
            C.prop_CasePorte (C.Carte 50 50 (C.C 0 0) (M.singleton (C.C 0 0) C.PorteNS))
            `shouldBe` False
        it "Une bonne carte en lecture" $ do
...
fullSpec = do
    describe "Verifie toute les propriétées d'une Carte" $ do
        ...
        it "Lecture d'une mauvaise carte (taille)" $ do
            C.prop_inv_Carte (C.readCarte "T=T\n|X|\n==")
            `shouldBe` False
        it "Lecture d'une mauvaise carte (entree)" $ do
            C.prop_inv_Carte (C.readCarte "T=T\n| |\n===")
            `shouldBe` False
